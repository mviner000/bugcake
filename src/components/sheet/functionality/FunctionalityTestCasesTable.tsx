// old src/components/sheet/functionality/FunctionalityTestCasesTable.tsx
...
  modules, // <--- ADDED: Destructure the modules prop
...
  // Fetch all test cases to get counts per status (unfiltered by workflow status)
  const allTestCasesData = useQuery(
    api.myFunctions.getTestCasesForSheet,
    sheetId ? { sheetId } : "skip"
  );

  // Safely extract all functionality test cases and calculate counts/total
...
        // FIX: Assert the trimmed string as the correct Convex Id type
        // The new select element ensures this field now contains a valid module ID or is empty string (falsy)
...
                        </select>
                      </td>
                      {/* Module - New | MODIFIED TO USE SELECT */}
                      <td
                        data-column="module"
                        style={{ width: `${getColumnWidth("module", 150)}px` }}
                        className="border border-gray-300 px-3 py-2"
